...
# 	def __repr__(self):
# 		return "ProfilePics(emp_contact={emp_contact},img_name={img_name})"
...
class Teachers(db.Model):
	...
	def __repr__(self):
		return f"Teacher(full_name={self.full_name},content_subject={self.content_subject},eligibility={self.eligibility},clg_street_name={self.clg_street_name},clg_state={self.clg_state},clg_city={self.clg_city},clg_contry={self.clg_contry},clg_zip_code={self.clg_zip_code},clg_code={self.clg_code},emp_street_name={self.emp_street_name},emp_state={self.emp_state},emp_city={self.emp_city},emp_contry={self.emp_contry},emp_zip_code={self.emp_zip_code},emp_contact={self.emp_contact},emp_alt_contact={self.emp_alt_contact},issue_date={self.issue_date},joining_date={self.joining_date},fee={self.fee},transact_date={self.transact_date},reciept_num={self.reciept_num},blood_grp={self.blood_grp})"
...
class Admins(db.Model):
	...
	def __repr__(self):
		return f"AdminLog(full_name={self.name},email_id={self.email_id},password={self.password})"
...
# img_post_args.add_argument('emp_contact', type=str, help="Employee contact number is required", required=True)
# img_post_args.add_argument('emp_contact', type=str, help="Employee contact number is required", required=True)
...
class Admin(Resource):
	@marshal_with(resource_fields)
	def get(self,emp_contact=None):
		if emp_contact and emp_contact != 0:
			result = Teachers.query.filter_by(emp_contact=emp_contact).first()
			if not result:
...
class UploadImage(Resource):
   def post(self):
     ...
     if image_file is not None:
     	image_file.save("IMG_202")
     



api.add_resource(Teacher, "/register/success/<int:emp_contact>")  
api.add_resource(Admin, "/admin/dashboard/", "/admin/dashboard/<int:emp_contact>") #1st end point gets all the teachers, 2nd end point gets a particular teacher based on employee_contact
api.add_resource(Admin_log, "/login") 	
api.add_resource(UploadImage, "/dp") 
